// https://www.codewars.com/kata/560a4962c0cc5c2a16000068/javascript
...
function eqSumPowdig(hMax, exp) {
  //Splits the number, raises each number by exponent and return the sum.
  function calc(num) {
    return String(num)
      .split('')
      .reduce((acc, c) => acc + Math.pow(+c, exp), 0);
  }
  //Ouput Array
  let arr = [];

  for (let i = 2; i <= hMax; i++) {
    //If the number is equal to the calculated number, push number to the output array.
    if (i === calc(i)) {
      arr.push(i);
    }
  }
