...
//   let code = String(n).repeat(Math.ceil(str.length / String(n).length));
//   return [...str].map((el, index) => alpha.indexOf(el) + +code[index]);
// }
...
function encode(str, n) {
  ...
  return Array.from(
    str,
    (el, index) => el.charCodeAt(0) - 96 + +code[index % code.length]
  );
}
